...
// Encode implements json.Marshaler.
func (s *RefreshKeyResponse) Encode(e *jx.Encoder) {
	e.ObjStart()
	s.encodeFields(e)
	e.ObjEnd()
}
...
// encodeFields encodes fields.
func (s *RefreshKeyResponse) encodeFields(e *jx.Encoder) {
	{
		e.FieldStart("refresh_key")
		e.Int64(s.RefreshKey)
	}
}

var jsonFieldsNameOfRefreshKeyResponse = [1]string{
	0: "refresh_key",
}

// Decode decodes RefreshKeyResponse from json.
func (s *RefreshKeyResponse) Decode(d *jx.Decoder) error {
	if s == nil {
		return errors.New("invalid: unable to decode RefreshKeyResponse to nil")
	}
	var requiredBitSet [1]uint8
	...
	if err := d.ObjBytes(func(d *jx.Decoder, k []byte) error {
		...
		case "refresh_key":
			...
			if err := func() error {
				v, err := d.Int64()
				s.RefreshKey = int64(v)
				if err != nil {
					return err
				}
				return nil
			}(); err != nil {
				return errors.Wrap(err, "decode field \"refresh_key\"")
			}
		default:
			return d.Skip()
		}
		...
		return errors.Wrap(err, "decode RefreshKeyResponse")
	}
	// Validate required fields.
	var failures []validate.FieldError
	for i, mask := range [1]uint8{
		...
		if result := (requiredBitSet[i] & mask) ^ mask; result != 0 {
			...
			for bitN := 0; bitN < missed; bitN++ {
				...
				fieldIdx := i*8 + bitIdx
				var name string
				if fieldIdx < len(jsonFieldsNameOfRefreshKeyResponse) {
					name = jsonFieldsNameOfRefreshKeyResponse[fieldIdx]
				} else {
					name = strconv.Itoa(fieldIdx)
				}
				failures = append(failures, validate.FieldError{
...
// MarshalJSON implements stdjson.Marshaler.
func (s *RefreshKeyResponse) MarshalJSON() ([]byte, error) {
	e := jx.Encoder{}
	s.Encode(&e)
	return e.Bytes(), nil
}
...
// UnmarshalJSON implements stdjson.Unmarshaler.
func (s *RefreshKeyResponse) UnmarshalJSON(data []byte) error {
	d := jx.DecodeBytes(data)
	return s.Decode(d)
}
