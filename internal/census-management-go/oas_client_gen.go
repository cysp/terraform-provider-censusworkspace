...
type Invoker interface {
	...
	// RefreshDatasetColumns invokes refreshDatasetColumns operation.
	//
...
//
// POST /api/v1/datasets/{dataset_id}/refresh_columns
func (c *Client) RefreshDatasetColumns(ctx context.Context, params RefreshDatasetColumnsParams) (*RefreshKeyResponseStatusCode, error) {
	res, err := c.sendRefreshDatasetColumns(ctx, params)
	return res, err
}

func (c *Client) sendRefreshDatasetColumns(ctx context.Context, params RefreshDatasetColumnsParams) (res *RefreshKeyResponseStatusCode, err error) {
...

	{
		...
		var satisfied bitset
		{
			...
			switch err := c.securityWorkspaceApiKey(ctx, RefreshDatasetColumnsOperation, r); {
			case err == nil: // if NO error
				satisfied[0] |= 1 << 0
			case errors.Is(err, ogenerrors.ErrSkipClientSecurity):
				// Skip this security.
	...
	result, err := decodeRefreshDatasetColumnsResponse(resp)
	if err != nil {
		return res, errors.Wrap(err, "decode response")
	}
