...
import (
	...
	provider "github.com/cysp/terraform-provider-censusworkspace/internal/provider"
...
func TestBigQuerySourceConnectionDetailsUnknown(t *testing.T) {
	t.Parallel()

	ctx := t.Context()

	value := provider.NewBigQuerySourceConnectionDetailsUnknown()
...
func TestBigQuerySourceConnectionDetailsNull(t *testing.T) {
	t.Parallel()

	ctx := t.Context()

	value := provider.NewBigQuerySourceConnectionDetailsNull()
...
func TestBigQuerySourceConnectionDetailsInvalid(t *testing.T) {
	t.Parallel()
	...
	for name, attributes := range testcases {
		t.Run(name, func(t *testing.T) {
			...
			value, diags := provider.NewBigQuerySourceConnectionDetailsKnownFromAttributes(ctx, attributes)
...
func TestBigQuerySourceConnectionDetailsConversion(t *testing.T) {
	t.Parallel()

	ctx := t.Context()

	values := []provider.AttrValueWithToObjectValue{
		provider.NewBigQuerySourceConnectionDetailsKnown(),
		DiagsNoErrorsMust(provider.NewBigQuerySourceConnectionDetailsKnownFromAttributes(ctx, map[string]attr.Value{
