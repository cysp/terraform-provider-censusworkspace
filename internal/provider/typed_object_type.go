...
func (t TypedObjectType[T]) TerraformType(ctx context.Context) tftypes.Type {
	tfAttributeTypes := make(map[string]tftypes.Type, len(t.attributeTypes))

	for key, attrType := range t.attributeTypes {
		tfAttributeTypes[key] = attrType.TerraformType(ctx)
	}

	return tftypes.Object{AttributeTypes: tfAttributeTypes}
}
